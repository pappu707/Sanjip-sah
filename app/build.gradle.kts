...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.jarvisassistant.vxqwla"
    minSdk = 24
    targetSdk = 35
    versionCode = 1
    versionName = "1.0"
...
tasks.register<Copy>("copyApkOutputsNoDot") {
    dependsOn("assembleDebug")
    from(layout.buildDirectory.file("outputs/apk/debug/app-debug.apk"))
    into(rootProject.file("build-outputs"))
    rename { "app-debug.apk" }
...
tasks.register("dispatchApks") {
    dependsOn("copyApkOutputs", "copyApkOutputsNoDot", "copyApkToDownload")
}
